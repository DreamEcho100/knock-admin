...
const MarketingPopUp = (props) => {
  ...
  const editPopUpmutation = useMutation({
    mutationFn: (event) => {
      ...
      return fetch(`${process.env.NEXT_PUBLIC_KNOCK_URL_API}/ui/edit-popup`, {
        method: "PUT",
        headers: {
    ...
    onSuccess: (result) => {
      setTimeout(() => toast(result.message), 0), setOpenEditMode(false);
    },
    onError: (result) =>
  ...
  const resetPopUpmutation = useMutation({
    mutationFn: (event) => {
      ...
      return fetch(`${process.env.NEXT_PUBLIC_KNOCK_URL_API}/ui/reset-popup`, {
        method: "GET",
        headers: {
          "Content-type": "application/json",
          Authorization: accessToken,
  ...
  return (
    <Dialog
      ...
      setIsOpen={(isOpen) =>
        generalStore.getState().setIsVisible("marketingPopup", isOpen)
      }
    >
      {openEditMode ? (
        ...
        <div className=" m-4 flex justify-center">
          {formValues.mainImageUrl ? (
            <CustomNextImage
              ...
              height={300}
              src={
                process.env.NEXT_PUBLIC_KNOCK_URL_API + formValues.mainImageUrl
              }
            />
          ) : (
            ""
          ...
          {openEditMode ? (
            <div className="flex flex-col   justify-center gap-4">
              <Button
                ...
                onClick={resetPopUpmutation.mutate}
              >
                Reset
              </Button>
