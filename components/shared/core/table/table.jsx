...
const Table = (props) => {
	...
	useEffect(() => {
		const getAdminsList = async () => {
			const response = await axios.get(
				`${process.env.NEXT_PUBLIC_KNOCK_URL_API}/admin/get-users`,
				{
					headers: {
						Authorization: JSON.parse(getCookie('user-access-token'))
							.accessToken
					}
				}
			);
	...
	const data = useMemo(() => props.admins, [props.admins]);
	...
	const columns = useMemo(
		() => [
					...
					return (
						<p>
							{moment(data?.lastLogin).format(
								windowSize.width < 1024 ? 'Do YYYY' : 'MMMM Do YYYY, h:mm:ss a'
							)}
						</p>
					);
				}
					...
					return (
						<p>
							{moment(data?.createdAt).format(
								windowSize.width < 1024 ? 'Do YYYY' : 'MMMM Do YYYY'
							)}
						</p>
					);
				}
			},
			{
				...
				accessor: (data) => {
					let adminEmail = [
						'support@pluginsthatknock.com',
						'mehdi.yanat3106@outlook.fr'
					];
					return (
	...
	return (
		<>
			<div className='max-w-[1200px] py-6 flex justify-between'>
				...
				<Button onClick={() => props.setIsAddAdminOpen(true)}>
					{' '}
					Add admin{' '}
				</Button>
			</div>
			<props.AddAdminMutiation
				setAdmins={props.setAdmins}
				isOpen={props.isAddAdminOpen}
